...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if DBEUG_skip_most_in_Unet_constructor:
            return
        ...
        self.out = nn.Sequential(
            normalization(ch),
            nn.SiLU(),
            zero_module(conv_nd(dims, model_channels, out_channels, 3, padding=1)), #model_channels=320, out_channels=3, dims=2
        )
    ...
    def forward(self, x, timesteps=None, context=None, y=None,return_features=False,**kwargs):
        ...
        dumper_var_prefix = 'refnet' if self.is_refNet else 'unet'
        ...
        for module in self.input_blocks:
            h = module(h, emb, context2)
            # global_dumper.save(h, f'{dumper_var_prefix}/input_blocks/{idx}')
            hs.append(h)
        if self.is_refNet:
            return
