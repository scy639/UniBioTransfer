...
class FrozenCLIPEmbedder(nn.Module):
    def __init__(self, version="openai/clip-vit-large-patch14"):
        ...
        self.model = CLIPModel.from_pretrained(version,local_files_only=local_files_only)
        del self.model.text_model
        self.resize = transforms.Resize((224, 224))
        # self.processor = CLIPProcessor.from_pretrained(version)
        self.tokenizer = CLIPTokenizer.from_pretrained(version,local_files_only=local_files_only)
    ...
    def forward_vit(self, image, resize=False):
        if resize:
    ...
    def encode_B(self, z):
        ...
        z = self.final_ln2(z)
        return z
    def forward(self, image, pooled_or_all_tokens = False):
        outputs = self.model.vision_model(pixel_values=image)
        if pooled_or_all_tokens:
            z = outputs.pooler_output
            z=self.model.visual_projection(z)
            # z=self.projection_back(z)
            z = z.unsqueeze(1)
            ...
            z = outputs.last_hidden_state # B, 257, 1024
            z=self.model.visual_projection(z) # B, 257, 768
            # z=self.projection_back(z)
            # z = z.unsqueeze(1)
        # shape unchanged:
        z = self.mapper2(z)
