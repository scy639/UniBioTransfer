...
class FaceMeshVisualizer:
    def __init__(self, 
            ...
            draw_pupil=True,
            line_thickness=2,
            img_256_mode :bool=True, # if false, mode 0-1
        ):
        self.mp_drawing = mp.solutions.drawing_utils
        mp_face_mesh = mp.solutions.face_mesh
        self.mp_face_mesh = mp_face_mesh
        self.forehead_edge = forehead_edge
        self.img_256_mode = img_256_mode

        DrawingSpec = mp.solutions.drawing_styles.DrawingSpec
        f_thick = line_thickness
        f_rad = 1
        
        eye_color = (10, 200, 150)
        eyebrow_color = (10, 220, 150)
        iris_color = (150 ,120, 100)
        # mouth_ob_color = (180, 10, 10)
        mouth_ob_color = eye_color
        mouth_ot_color = mouth_ob_color
        mouth_ib_color = (10, 180, 20)
        mouth_it_color = mouth_ib_color
        
        # Scale colors to 0-1 range if not in 256 mode
        if not self.img_256_mode:
            eye_color = tuple(c / 255.0 for c in eye_color)
            eyebrow_color = tuple(c / 255.0 for c in eyebrow_color)
            iris_color = tuple(c / 255.0 for c in iris_color)
            mouth_ob_color = tuple(c / 255.0 for c in mouth_ob_color)
            mouth_ot_color = tuple(c / 255.0 for c in mouth_ot_color)
            mouth_ib_color = tuple(c / 255.0 for c in mouth_ib_color)
            mouth_it_color = tuple(c / 255.0 for c in mouth_it_color)
        
        right_iris_draw = DrawingSpec(color=iris_color, thickness=f_thick, circle_radius=f_rad)
        right_eye_draw = DrawingSpec(color=eye_color, thickness=f_thick, circle_radius=f_rad)
        right_eyebrow_draw = DrawingSpec(color=eyebrow_color, thickness=f_thick, circle_radius=f_rad)
        left_iris_draw = DrawingSpec(color=iris_color, thickness=f_thick, circle_radius=f_rad)
        left_eye_draw = DrawingSpec(color=eye_color, thickness=f_thick, circle_radius=f_rad)
        left_eyebrow_draw = DrawingSpec(color=eyebrow_color, thickness=f_thick, circle_radius=f_rad)
        head_draw = DrawingSpec(color=(10, 200, 10) if self.img_256_mode else (10/255.0, 200/255.0, 10/255.0), thickness=f_thick, circle_radius=f_rad)
        nose_draw = DrawingSpec(color=eyebrow_color, thickness=f_thick, circle_radius=f_rad)
        
        mouth_draw_obl = DrawingSpec(color=mouth_ob_color, thickness=f_thick, circle_radius=f_rad)
        mouth_draw_obr = DrawingSpec(color=mouth_ob_color, thickness=f_thick, circle_radius=f_rad)
        
        mouth_draw_ibl = DrawingSpec(color=mouth_ib_color, thickness=f_thick, circle_radius=f_rad)
        mouth_draw_ibr = DrawingSpec(color=mouth_ib_color, thickness=f_thick, circle_radius=f_rad)
        
        mouth_draw_otl = DrawingSpec(color=mouth_ot_color, thickness=f_thick, circle_radius=f_rad)
        mouth_draw_otr = DrawingSpec(color=mouth_ot_color, thickness=f_thick, circle_radius=f_rad)
        
        mouth_draw_itl = DrawingSpec(color=mouth_it_color, thickness=f_thick, circle_radius=f_rad)
        mouth_draw_itr = DrawingSpec(color=mouth_it_color, thickness=f_thick, circle_radius=f_rad)
    ...
    def draw_points(self, image, landmark_list, drawing_spec, halfwidth: int = 2):
        ...
        for idx, landmark in enumerate(landmark_list.landmark):
            ...
            if not self.img_256_mode:  # if 0-1 mode, scale color values from 0-255 to 0-1
                if 0:  # default: if not img_256_mode, we assume input is 0-1, so no conversion
                    draw_color = tuple(c / 255.0 for c in draw_color)
            image[image_y - halfwidth : image_y + halfwidth, image_x - halfwidth : image_x + halfwidth, :] = draw_color
        

    def draw_landmarks(self, image_size, keypoints, normed=False, image=None):
        # print(f"{image_size=}") # 512, 512
        ini_size = [512, 512]
        if image is None:
            if self.img_256_mode:
                image = np.zeros([ini_size[1], ini_size[0], 3], dtype=np.uint8)
            else:
                image = np.zeros([ini_size[1], ini_size[0], 3], dtype=np.float32)
