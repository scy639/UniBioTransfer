...
def __ffhq_masks_to_faceParser_mask_detailed(mask, label_mode):
    ...
    if label_mode=="RF12_": # for RF12_ (non-RF12 modes count hat/cloth as background via `converted_mask = np.zeros_like(mask)`)
        max_label = int(np.max(mask))
        # print(f"__ff  {max_label=}")
        # cloth -> 20, hat -> 21
        cloth = np.equal(mask, 16)
        converted_mask[cloth] = 20
        hat = np.equal(mask, 18)
        converted_mask[hat] = 21
    return converted_mask
...
def vis_parsing_maps(image, parsing_anno, stride=1):
    ...
    if 0:
        len_= num_of_class + 1
        delta_ = 256/len_
        part_colors = [ [round(i*delta_), round(i*delta_), round(i*delta_)]  for i in range(len_)  ]
        print(f"{part_colors[0]=}")
        print(f"{part_colors[-1]=}")
    if 0:
        print(f"{len(part_colors)=}")
        print(f'{np.unique(vis_parsing_anno)=}')
    ...
    # vis_im = cv2.addWeighted(cv2.cvtColor(vis_im, cv2.COLOR_RGB2BGR), 0.1, vis_parsing_anno_color, 0.9, 0)
...
def faceParsing_demo(model, img, label_mode, model_name = "default"):
    """
    args:
    ...
    with torch.no_grad():
        if model_name == "default":
            ...
            if convert_to_seg12:
                seg = __ffhq_masks_to_faceParser_mask_detailed(seg,label_mode)
